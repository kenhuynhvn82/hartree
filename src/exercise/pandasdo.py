...
def merge_two_datasets_by_pandas(input_file1: str, input_file2: str, output_file: str):
    ...
    df['status_ARAP'] = np.where(df['status'] == 'ARAP', df['value'], 0)
    df['status_ACCR'] = np.where(df['status'] == 'ACCR', df['value'], 0)
    
    all_group_columns = ['legal_entity', 'counter_party', 'tier']
    gf2 = df.groupby(all_group_columns, as_index=False).agg({'rating':max, 'status_ARAP':sum, 'status_ACCR':sum})
    ...
    for attr in [['legal_entity'], ['legal_entity', 'counter_party'], ['counter_party'], ['tier']]:
        ...
        dfByAttr = df.groupby(attr, as_index=False).agg({'rating':max, 'status_ARAP':sum, 'status_ACCR':sum}).assign(**dict.fromkeys(extra_attr, 'Total'))
